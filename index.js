...
// ✅ FIX #3: Validate battle creation server-side
exports.validateBattleCreation = onRequest((req, res) => {
  ...
  cors(req, res, async () => {
    ...
    try {
      // ✅ SECURITY FIX #1: Verify Firebase ID token FIRST
      // This ensures the request is authenticated and gets the real user ID
      const decoded = await verifyToken(req);
      authenticatedUid = decoded.uid;
    } catch (error) {
      // If token verification fails, reject immediately
      functions.logger.warn('[validateBattleCreation] Token verification failed', {
        error: error.message,
        timestamp: new Date().toISOString()
      });
    ...
    try {
      // ✅ SECURITY FIX #2: Extract userId from request body
      ...
      // ✅ SECURITY FIX #3: Ensure user can only check their OWN battle limits
      // This prevents enumeration attacks and unauthorized checks of other users' limits
      if (userId !== authenticatedUid) {
        functions.logger.warn('[validateBattleCreation] Cross-user attack attempt blocked', {
          authenticatedUid,
        ...
        return res.status(403).json({
          allowed: false,
          message: 'You can only check your own battle limits'
        });
      }

      // ✅ SECURITY FIX #4: Add rate limiting to prevent probing/spam
      try {
        await checkRateLimit(authenticatedUid, 'validateBattleCreation', 30);
      ...
      // Get user from Firestore (source of truth)
      ...
      const MAX_BATTLES_FREE = 0;
      const MAX_BATTLES_BATTLE = 5;
      const MAX_BATTLES_BATTLE_PRO = 19;
      const MAX_BATTLES_UNLIMITED = 999999;

      let allowedBattles = MAX_BATTLES_FREE;

      if (isPremium) {
        if (premiumPlan === 'battle') {
          allowedBattles = MAX_BATTLES_BATTLE;
        } else if (premiumPlan === 'battle_pro') {
          allowedBattles = MAX_BATTLES_BATTLE_PRO;
        } else if (premiumPlan === 'battle_academy') {
          allowedBattles = 29;
        } else {
          allowedBattles = MAX_BATTLES_UNLIMITED;
        }
      }

      // Get actual battle count from Firestore (NOT from client)
      const thisMonth = new Date().getFullYear() + '_' + new Date().getMonth();
      const battleCountSnap = await db.collection('battleCreatorUsage').doc(userId).get();

      let actualCount = 0;
      if (battleCountSnap.exists()) {
        const data = battleCountSnap.data();
        if (data.month === thisMonth) {
          actualCount = data.count || 0;
        }
      }

      // Validate: use Firestore count, not client-supplied count
      const canCreate = actualCount < allowedBattles;
      ...
      if (canCreate) {
        return res.status(200).json({
          ...
          actualCount,
          allowedBattles,
          message: `Battle creation allowed (${actualCount + 1}/${allowedBattles})`
        });
      } else {
        return res.status(403).json({
          allowed: false,
          actualCount,
          allowedBattles,
          message: `Battle limit reached (${actualCount}/${allowedBattles}). Upgrade your plan.`
        });
      }
    } catch (error) {
      functions.logger.error('[validateBattleCreation] Error:', {
        authenticatedUid,
        error: error.message,
        code: error.code,
        timestamp: new Date().toISOString()
      ...
      return res.status(500).json({
        allowed: false,
        message: 'Server error validating battle creation'
      });
    }
  });
});
...
// ═══════════════════════════════════════════════════════════════════════════════
// ✅ FIX #4: Cloud Function for secure coin rewards (backend ONLY)
// ═══════════════════════════════════════════════════════════════════════════════
// POST /api/claim-battle-reward
// Claim coins reward for completing a battle (prevents duplication)
// Request body: { battleId, userId, rewardAmount }
// Only Cloud Function can call this - coins NEVER awarded from client
exports.claimBattleReward = onRequest((req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  ...
  cors(req, res, async () => {
    ...
    try {
      // ✅ SECURITY FIX #1: Verify Firebase ID token FIRST
      // Extract the real user ID from the verified token, not from request body
      const decoded = await verifyToken(req);
      authenticatedUid = decoded.uid;
    } catch (error) {
      // If token verification fails, reject immediately
      functions.logger.warn('[claimBattleReward] Token verification failed', {
        error: error.message,
        timestamp: new Date().toISOString()
      });
      return res.status(401).json({
    ...
    try {
      // ✅ SECURITY FIX #2: Extract from body but validate against verified token
      const { battleId, userId, rewardAmount } = req.body;

      // ✅ SECURITY FIX #3: User can ONLY claim rewards for themselves
      // This prevents one user from claiming rewards on behalf of another
      if (userId !== authenticatedUid) {
        functions.logger.warn('[claimBattleReward] Cross-user reward claim attempt blocked', {
          authenticatedUid,
          requestedUserId: userId,
          battleId,
          timestamp: new Date().toISOString()
        ...
        return res.status(403).json({
          success: false,
          error: 'You can only claim rewards for yourself'
        });
      }

      // Validate basic inputs
      if (!battleId || !userId || !rewardAmount || rewardAmount <= 0) {
        return res.status(400).json({
          success: false,
          error: 'Invalid request: battleId, userId, and rewardAmount required'
        });
      }

      // ✅ SECURITY FIX #4: Add rate limiting to prevent reward spam
      try {
        await checkRateLimit(authenticatedUid, 'claimBattleReward', 50);
      } catch (rateLimitError) {
        return res.status(429).json({
          success: false,
          error: 'Too many reward claims. Please try again later.'
        });
      }

      // ✅ SECURITY FIX #5: Validate reward amount is within reasonable bounds
      const MAX_REWARD_PER_CLAIM = 500; // Adjust based on your game design
      if (rewardAmount > MAX_REWARD_PER_CLAIM) {
        functions.logger.warn('[claimBattleReward] Excessive reward amount attempted', {
          authenticatedUid,
          battleId,
          requestedAmount: rewardAmount,
          maxAllowed: MAX_REWARD_PER_CLAIM,
          timestamp: new Date().toISOString()
        });
        return res.status(400).json({
          success: false,
          error: 'Reward amount exceeds maximum allowed'
        });
      }

      // ✅ SECURITY: Verify battle exists and user participated
      const battleRef = db.collection('publicBattles').doc(battleId);
      ...
      if (!battleData.players || !battleData.players[userId]) {
        return res.status(403).json({
          success: false,
          error: 'User did not participate in this battle'
        });
      }

      // ✅ SECURITY FIX #6: Verify battle is completed before allowing reward claim
      // Status must be 'completed' or 'ended', not 'in_progress' or 'cancelled'
      const validStatuses = ['completed', 'ended'];
      if (!battleData.status || !validStatuses.includes(battleData.status)) {
        functions.logger.warn('[claimBattleReward] Attempted claim from incomplete battle', {
          authenticatedUid,
          battleId,
        ...
        return res.status(400).json({
          success: false,
          error: 'Battle must be completed before claiming rewards'
        });
      }

      // ✅ SECURITY: Check if reward already claimed (prevent duplication)
      const rewardKey = `claimed_${battleId}`;
      const userCoinsRef = db.collection('userCoins').doc(userId);

      // ✅ SECURITY: Only Cloud Function writes coins (uses admin SDK, bypasses rules)
      // Use a transaction to ensure atomicity and check for duplicate claims
      await db.runTransaction(async (transaction) => {
        const userCoinsSnap = await transaction.get(userCoinsRef);

        // Check if reward already claimed within transaction
        if (userCoinsSnap.exists() && userCoinsSnap.data()[rewardKey]) {
          throw { 
            status: 409, 
        ...
        if (!userCoinsSnap.exists()) {
          // Create userCoins doc if it doesn't exist
          ...
          // Update existing doc
          transaction.update(userCoinsRef, {
      ...
      functions.logger.info('[claimBattleReward] Coins awarded successfully', {
        authenticatedUid,
        userId,
        battleId,
        rewardAmount,
      ...
      // ✅ Enhanced error logging with authenticatedUid
      functions.logger.error('[claimBattleReward] Error claiming reward', {
        authenticatedUid,
        error: error.message,
        code: error.code,
        status: error.status,
      ...
      const errorMessage = error.message || 'Server error claiming reward';
