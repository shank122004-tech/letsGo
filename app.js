...
const ModalStateManager = {
  ...
  savePreviousState() {
    // Check which modals are currently open with proper visibility checks
    const baModal = document.getElementById('ba-modal');
    const lbModal = document.getElementById('lb-modal');
    
    // Check if modals are actually visible (not just in DOM)
    const baVisible = baModal && 
                      baModal.style.display !== 'none' && 
                      baModal.style.display !== '' &&
                      baModal.style.visibility !== 'hidden' &&
                      (baModal.classList.contains('open') || baModal.style.display === 'flex');
    
    const lbVisible = lbModal && 
                      lbModal.style.display !== 'none' && 
                      lbModal.style.display !== '' &&
                      lbModal.style.visibility !== 'hidden' &&
                      (lbModal.classList.contains('open') || lbModal.style.display === 'flex');
    
    // Set state based on what's actually visible
    if (baVisible) {
      this.previousState = 'battleArena';
    } else if (lbVisible) {
      this.previousState = 'leaderboard';
    } else {
      this.previousState = 'home';
    }
    
    console.log('[ModalStateManager] Saved state:', this.previousState);
  },
  ...
  restorePreviousState() {
    ...
    console.log('[ModalStateManager] Restoring state:', this.previousState);
    
    // Show the appropriate modal based on saved state
    if (this.previousState === 'battleArena' && baModal) {
      ...
      console.log('[ModalStateManager] Restored battle arena');
    } else if (this.previousState === 'leaderboard' && lbModal) {
      ...
      console.log('[ModalStateManager] Restored leaderboard');
    } else {
      console.log('[ModalStateManager] Restored to home (all modals hidden)');
    }
    // If previousState is 'home', just hide all modals and let home show through
...
window.ModalStateManager = ModalStateManager;

// ─── GLOBAL MODAL CLOSE HANDLERS ──────────────────────────────────────────
window.closeRankPredictorModal = function() {
  const modal = document.getElementById('rankPredictorModal');
  if (modal) {
    ...
    if (typeof ModalStateManager !== 'undefined') {
      setTimeout(() => {
        ModalStateManager.restorePreviousState();
      }, 100);
    }
  }
};
...
function _buildTeacherAdModal() {
  ...
  modal.innerHTML = `
    <div style="background:linear-gradient(145deg,#0f0a25,#1a0f3a);border:1px solid rgba(255,107,157,0.3);border-radius:24px;padding:32px 28px;max-width:380px;width:calc(100%-32px);position:relative;text-align:center;box-shadow:0 24px 80px rgba(108,99,255,0.35);">
      ...
      <p style="font-size:13px;color:var(--text-secondary);line-height:1.6;margin-bottom:20px;">Watch <strong style="color:#FF6B9D">5 short sponsored videos</strong> to unlock AI Teacher voice mode for <strong style="color:#5b46d4">30 minutes</strong> with up to <strong style="color:#4ade80">15 questions</strong>.</p>
      ...
      <div id="teacherAdProgress" style="font-size:13px;color:var(--text-secondary);margin-bottom:20px;">0 / 5 ads watched</div>
      ...
      <button onclick="handlePayment && handlePayment('ssc')" style="width:100%;padding:10px;margin-top:10px;background:transparent;color:var(--text-secondary);border:1px solid rgba(108,99,255,0.2);border-radius:12px;font-size:13px;cursor:pointer;">Or upgrade to Premium for unlimited access ⭐</button>
    </div>`;
...
function loadUserState(uid) {
  ...
  // Update premium upgrade screen based on loaded status
  setTimeout(() => updatePremiumUpgradeScreen(), 500);
...
/* ✅ FIX #1: Load Premium Status from Firebase on login, app startup, and app resume */
async function loadPremiumStatusFromFirebase(uid) {
  try {
    if (!window._firebaseDb || !window._firebaseFns) return;
    ...
    if (userSnap.exists()) {
      const userData = userSnap.data();
      const firebaseIsPremium = userData.isPremium === true;
      const firebasePlan = userData.premiumPlan || null;
      const firebaseExpiry = userData.premiumExpiry || userData.premiumExpiresAt || 0;
      
      // ✅ Firestore is ALWAYS the source of truth for premium status
      // Check if premium is valid (not expired)
      const isValid = firebaseIsPremium && firebaseExpiry > Date.now();
      
      if (isValid) {
        // Premium is active and valid
        state.isPremium = true;
        state.premiumPlan = firebasePlan;
        state.premiumExpiry = firebaseExpiry;
        
        // Sync to localStorage (cache only, not source of truth)
        const p = _up(uid);
        localStorage.setItem(p + 'premium', 'true');
        localStorage.setItem(p + 'premium_plan', firebasePlan || '');
        localStorage.setItem(p + 'premium_expires', firebaseExpiry.toString());
        
        // Update UI
        setTimeout(() => {
          try {
            if (typeof updatePremiumUpgradeScreen === 'function') updatePremiumUpgradeScreen();
            if (typeof updateUserUI === 'function') updateUserUI();
            if (typeof updateProfileUI === 'function') updateProfileUI();
          } catch(e) {}
        }, 50);
      } else {
        // Premium is expired or not set - clear everything
    ...
    // Silently fail — will use localStorage value
  }
}

/* ✅ FIX #2: Load Coins from Firebase and sync to localStorage */
async function loadCoinsFromFirebase(uid) {
  try {
    if (!window._firebaseDb || !window._firebaseFns) return;
    
    const { doc, getDoc } = window._firebaseFns;
    
    // Load coins from Firestore (source of truth)
    const coinsRef = doc(window._firebaseDb, 'userCoins', uid);
    const coinsSnap = await getDoc(coinsRef);
    
    if (coinsSnap.exists()) {
      const firebaseCoins = coinsSnap.data().coins || 0;
      
      // ✅ Firestore is ALWAYS the source of truth
      // Never trust localStorage - always overwrite with Firestore value
      const coinsData = {
        coins: firebaseCoins,
        lastUpdated: Date.now()
      };
      localStorage.setItem('sscai_u:' + uid + ':coins', JSON.stringify(coinsData));
      
      // Update state for in-memory access
      state.coins = firebaseCoins;
      
      // Update ALL coin-related UIs immediately
      setTimeout(() => {
        try {
          if (typeof refreshCoinUI === 'function') refreshCoinUI();
          if (typeof updateUserUI === 'function') updateUserUI();
    ...
    // Silently fail - will use cached localStorage value
  }
}

/* ✅ FIX #4: Claim battle reward from Cloud Function (backend ONLY) */
async function claimBattleRewardFromBackend(battleId, userId, rewardAmount) {
  try {
    if (!window._firebaseAuth || !window._firebaseAuth.currentUser) {
      console.error('[claimBattleReward] User not authenticated');
    ...
    // Call Cloud Function to claim reward (backend validates everything)
    const response = await fetch('https://us-central1-rankgpt-f8a64.cloudfunctions.net/claimBattleReward', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (data.success) {
      // Reload coins from Firestore to sync with server
      await loadCoinsFromFirebase(userId);
      
      // Show reward notification
      if (typeof showToast === 'function') {
        showToast(`🎉 Earned ${rewardAmount} coins!`);
      }
      
      return { success: true, message: data.message };
    } else {
      console.warn('[claimBattleReward] Reward claim failed:', data.error);
      return { success: false, error: data.error };
    }
  } catch (error) {
    console.error('[claimBattleReward] Error claiming reward:', error);
...
async function sendMessage() {
  ...
  // Hide motion animation when entering workspace
  const motionWrapper = document.getElementById('ai-motion-wrapper');
  if (motionWrapper) {
    motionWrapper.style.display = 'none';
    motionWrapper.style.opacity = '0';
    motionWrapper.style.visibility = 'hidden';
  }
...
function showMaintenanceOverlay() {
  ...
  el.innerHTML = `
    <div style="max-width:420px; background:#15131f; border:1px solid rgba(108,99,255,0.3);
      ...
      <p style="color:var(--text-secondary); font-size:14px; line-height:1.6; margin:0 0 20px;">
...
function toggleTheme() { 
  // Don't allow theme changes while profile is open
  if (window._profileOpen) {
    if (typeof showToast === 'function') showToast('Close profile to change theme');
    return;
  }
...
function openProfileModal() { 
  ...
  if (!window._savedTheme) {
    window._savedTheme = document.documentElement.getAttribute('data-theme') || 'light';
  }
  document.documentElement.setAttribute('data-theme', 'dark');
  window._profileOpen = true;
  updateProfileUI(); 
...
function closeProfileModal() { 
  ...
  if (window._savedTheme) {
    document.documentElement.setAttribute('data-theme', window._savedTheme);
    window._savedTheme = null;
  }
  window._profileOpen = false;
  dom.profileModal.classList.remove('active'); 
}

// ═══════════════════════════════════════════════════════════════════════════════
// Show/Hide Premium Upgrade Screen based on User Premium Status
// ═══════════════════════════════════════════════════════════════════════════════

function updatePremiumUpgradeScreen() {
  try {
    const upgradeScreen = document.getElementById('premiumUpgradeScreen');
    const welcomeScreen = document.getElementById('welcomeScreen');
    
    // If elements don't exist, initialize motion animation and return
    if (!upgradeScreen || !welcomeScreen) {
      console.warn('[Premium] Elements not found - initializing motion animation');
      if (typeof window.initProfessionalAIMotion === 'function') {
        setTimeout(() => window.initProfessionalAIMotion(), 100);
      }
      return;
    }
    
    // Check if user is premium
    const isPremium = state.isPremium || (state.premiumPlan && state.premiumPlan !== 'free');
    
    console.log('[Premium] Status Check - isPremium:', isPremium, 'state.isPremium:', state.isPremium, 'state.premiumPlan:', state.premiumPlan);
    
    if (isPremium) {
      // Premium user - show welcome screen, hide upgrade
      upgradeScreen.style.display = 'none !important';
      upgradeScreen.style.visibility = 'hidden';
      upgradeScreen.style.pointerEvents = 'none';
      welcomeScreen.style.display = 'block';
      welcomeScreen.style.visibility = 'visible';
      welcomeScreen.style.pointerEvents = 'auto';
      welcomeScreen.classList.remove('ai-motion-active');
      console.log('[Premium] Showing welcome screen for PREMIUM user');
    } else {
      // Free user - show professional AI motion animation
      upgradeScreen.style.display = 'none !important';
      upgradeScreen.style.visibility = 'hidden';
      upgradeScreen.style.pointerEvents = 'none';
      welcomeScreen.style.display = 'block';
      welcomeScreen.style.visibility = 'visible';
      welcomeScreen.style.pointerEvents = 'auto';
      // Initialize professional AI motion for non-premium users
      if (typeof window.initProfessionalAIMotion === 'function') {
        setTimeout(() => window.initProfessionalAIMotion(), 100);
      }
      console.log('[Premium] Showing professional AI motion for FREE user');
    }
  } catch(e) {
    console.error('[Premium] Error updating screen:', e);
    // Fallback: try to show motion animation
    if (typeof window.initProfessionalAIMotion === 'function') {
      setTimeout(() => window.initProfessionalAIMotion(), 100);
    }
  }
...
function onPremiumStatusChange() {
  updatePremiumUpgradeScreen();
}

// ═══════════════════════════════════════════════════════════════════════════════
// TEST FUNCTIONS (exposed to window for debugging)
// ═══════════════════════════════════════════════════════════════════════════════

window._testShowUpgradeScreen = function() {
  const upgradeScreen = document.getElementById('premiumUpgradeScreen');
  const welcomeScreen = document.getElementById('welcomeScreen');
  if (upgradeScreen) upgradeScreen.style.display = 'block';
  if (welcomeScreen) welcomeScreen.style.display = 'none';
  console.log('[TEST] Upgrade screen shown');
};

window._testShowWelcomeScreen = function() {
  const upgradeScreen = document.getElementById('premiumUpgradeScreen');
  const welcomeScreen = document.getElementById('welcomeScreen');
  if (upgradeScreen) upgradeScreen.style.display = 'none';
  if (welcomeScreen) welcomeScreen.style.display = 'block';
  console.log('[TEST] Welcome screen shown');
};

window._testTogglePremium = function(isPremium) {
  state.isPremium = isPremium;
  state.premiumPlan = isPremium ? 'premium' : null;
  updatePremiumUpgradeScreen();
  console.log('[TEST] Premium status set to:', isPremium);
};

window._testCheckElements = function() {
  const upgradeScreen = document.getElementById('premiumUpgradeScreen');
  const welcomeScreen = document.getElementById('welcomeScreen');
  console.log('[TEST] Upgrade screen exists:', !!upgradeScreen);
  console.log('[TEST] Welcome screen exists:', !!welcomeScreen);
  console.log('[TEST] Current state.isPremium:', state.isPremium);
  console.log('[TEST] Current state.premiumPlan:', state.premiumPlan);
};
...
function updateProfileUI() {
  if (state.user) {
    ...
    updateXPDisplay(state.user.uid);
  } else {
...
function updateXPDisplay(uid, totalXP) {
  try {
    ...
    // Get XP from localStorage or parameter
    let xpValue = totalXP;
    if (!xpValue) {
      const xpStorageKey = `sscai_u:${uid}:totalXP`;
      xpValue = parseInt(localStorage.getItem(xpStorageKey) || '0');
    }
    ...
    // Update all XP display elements
...
// ═══════════════════════════════════════════════════════════════════════════════
// MOCK TEST STATS - Updates profile with test results
// ═══════════════════════════════════════════════════════════════════════════════

function processMockTestResults(results) {
  if (!results || !state.user) return;
  const uid = state.user.uid || state.user.id || state.userId;
  ...
  try {
    ...
    updateMockTestAggregateStats(uid, history);
    setTimeout(() => {
      updateProfileUI();
      updateAnalyticsDisplay(uid);
      updateRankPredictorDisplay(uid);
    }, 100);
  } catch(e) {
...
function updateMockTestAggregateStats(uid, history) {
  if (!uid || history.length === 0) return;
  try {
    let totalTests = history.length;
    let totalAccuracy = 0;
    let totalXP = 0;
    let categoryWiseStats = {};
    history.forEach(test => {
      totalAccuracy += test.accuracy || 0;
      totalXP += test.xpEarned || 0;
      const examType = test.examType || test.examName;
      if (!categoryWiseStats[examType]) {
        categoryWiseStats[examType] = { tests: 0, totalAccuracy: 0, avgAccuracy: 0, bestScore: 0 };
      }
      categoryWiseStats[examType].tests++;
      categoryWiseStats[examType].totalAccuracy += test.accuracy || 0;
      categoryWiseStats[examType].bestScore = Math.max(categoryWiseStats[examType].bestScore, test.score || 0);
    });
    const avgAccuracy = totalTests > 0 ? Math.round(totalAccuracy / totalTests) : 0;
    Object.keys(categoryWiseStats).forEach(key => {
      const stats = categoryWiseStats[key];
      stats.avgAccuracy = Math.round(stats.totalAccuracy / stats.tests);
    });
    const statsKey = `sscai_u:${uid}:mock_stats`;
    const aggregateStats = { totalTests, avgAccuracy, totalXP, categoryWiseStats, lastUpdated: Date.now() };
    localStorage.setItem(statsKey, JSON.stringify(aggregateStats));
    
    // Also save totalXP separately for easy access in profile
    const xpStorageKey = `sscai_u:${uid}:totalXP`;
    localStorage.setItem(xpStorageKey, String(totalXP));
    
    // Update XP display immediately
    updateXPDisplay(uid, totalXP);
  } catch(e) {
    console.error('[Stats Update] Error:', e);
  }
}

function updateAnalyticsDisplay(uid) {
  if (!uid) return;
  try {
    const historyKey = `sscai_u:${uid}:mock_history`;
    const statsKey = `sscai_u:${uid}:mock_stats`;
    ...
    if (!modal || !stats || stats.totalTests === 0) return;
    const testsEl = modal.querySelector('[data-analytics-tests]');
    if (testsEl) testsEl.textContent = stats.totalTests;
    const accuracyEl = modal.querySelector('[data-analytics-accuracy]');
    if (accuracyEl) accuracyEl.textContent = stats.avgAccuracy + '%';
    const xpEl = modal.querySelector('[data-analytics-xp]');
    if (xpEl) xpEl.textContent = stats.totalXP;
    if (history.length >= 2) {
      const recentTests = history.slice(-5);
      const oldTests = history.slice(0, Math.min(5, history.length - 5));
      const recentAvg = Math.round(recentTests.reduce((sum, t) => sum + (t.accuracy || 0), 0) / recentTests.length);
      const oldAvg = oldTests.length > 0 ? Math.round(oldTests.reduce((sum, t) => sum + (t.accuracy || 0), 0) / oldTests.length) : recentAvg;
      const improvement = recentAvg - oldAvg;
      const improvementEl = modal.querySelector('[data-analytics-improvement]');
      if (improvementEl) {
        improvementEl.textContent = (improvement >= 0 ? '+' : '') + improvement + '%';
        improvementEl.style.color = improvement >= 0 ? '#66bb6a' : '#ff6b6b';
      }
    }
  } catch(e) {
...
function calculateRealisticRank(uid) {
  if (!uid) return null;
  try {
    const historyKey = `sscai_u:${uid}:mock_history`;
    const history = JSON.parse(localStorage.getItem(historyKey) || '[]');
    ...
    // Assume 100-150 total questions per test (typical for competitive exams)
    const QUESTIONS_PER_TEST = 10; // Mock tests have 10 questions
    
    // Get recent tests for analysis
    ...
    recentTests.forEach(test => {
      totalQuestionsAttempted += (test.correctAnswers + test.wrongAnswers);
      totalCorrect += test.correctAnswers;
      totalWrong += test.wrongAnswers;
      totalUnattempted += test.unattempted;
    });
    
    const avgAccuracy = recentTests.length > 0 
      ? Math.round(recentTests.reduce((sum, t) => sum + (t.accuracy || 0), 0) / recentTests.length)
      : 0;
    
    const attemptRate = totalQuestionsAttempted / (recentTests.length * QUESTIONS_PER_TEST);
    const avgQuestionsAttempted = Math.round(totalQuestionsAttempted / recentTests.length);
    
    // Realistic rank calculation based on competitive exam patterns
    // Assuming ~50,000 candidates for most competitive exams
    let estimatedRank = 50000;
    let isInsufficientData = false;
    let confidence = 0;
    
    if (totalTests === 1 && avgQuestionsAttempted <= 3) {
      // Only 1 test with very few attempts - insufficient data
      isInsufficientData = true;
      estimatedRank = 45000 + Math.round(45000 * (1 - avgAccuracy / 100));
      confidence = 10;
    } else if (totalTests < 3) {
      // Less than 3 tests - low confidence
      isInsufficientData = true;
      
      // Formula: Base rank adjusted by accuracy and attempt rate
      const accuracyFactor = (avgAccuracy / 100) * 0.6;
      const attemptFactor = (attemptRate) * 0.4;
      const performanceFactor = accuracyFactor + attemptFactor;
      
      estimatedRank = Math.round(50000 * (1 - performanceFactor));
      confidence = 20 + (totalTests * 10);
    } else {
      // 3+ tests - better confidence
      const accuracyFactor = (avgAccuracy / 100) * 0.5;
      const attemptFactor = Math.min(attemptRate, 1) * 0.3;
      const consistencyBonus = totalTests > 5 ? 0.2 : 0.1;
      const performanceFactor = accuracyFactor + attemptFactor + consistencyBonus;
      
      estimatedRank = Math.round(50000 * (1 - performanceFactor));
      confidence = Math.min(70 + (totalTests * 5), 95);
    }
    
    // Ensure rank is within realistic bounds
    estimatedRank = Math.max(1, Math.min(50000, estimatedRank));
    
    // Calculate percentile (0-100, where 100 is top)
    const percentile = Math.max(1, 100 - Math.round((estimatedRank / 50000) * 100));
    
    return {
      estimatedRank,
      percentile,
      yourScore: avgAccuracy,
      questionsAttempted: avgQuestionsAttempted,
      attemptRate: Math.round(attemptRate * 100),
      totalTests,
      isInsufficientData,
      confidence,
      message: getRealisticMessage(totalTests, avgAccuracy, avgQuestionsAttempted)
    };
  } catch(e) {
...
function getRealisticMessage(totalTests, accuracy, questionsAttempted) {
  if (totalTests === 1 && questionsAttempted <= 3) {
    return `Take more tests (${3 - totalTests} more) with full attempts for accurate prediction`;
  } else if (totalTests < 3) {
    return `Low confidence: Complete ${3 - totalTests} more test(s) for better accuracy`;
  } else if (totalTests < 5) {
    return `Moderate confidence: More tests will improve prediction accuracy`;
  } else if (totalTests < 10) {
    return `Good confidence: Based on ${totalTests} tests`;
  } else {
    return `High confidence: Based on ${totalTests} tests`;
  }
}

function updateRankPredictorDisplay(uid) {
  if (!uid) return;
  try {
    const prediction = calculateRealisticRank(uid);
    const modal = document.getElementById('rankPredictorModal');
    ...
    // Update rank
    const rankEl = modal.querySelector('[data-rank-predictor-rank]');
    if (rankEl) {
      if (prediction.isInsufficientData) {
        rankEl.textContent = '~' + prediction.estimatedRank.toLocaleString();
    ...
    // Update percentile
    const percentileEl = modal.querySelector('[data-rank-predictor-percentile]');
    if (percentileEl) {
      if (prediction.isInsufficientData) {
        percentileEl.textContent = '~' + prediction.percentile + '%';
        percentileEl.style.opacity = '0.6';
      } else {
        percentileEl.textContent = prediction.percentile + '%';
        percentileEl.style.opacity = '1';
      }
    }
    
    // Update score
    const scoreEl = modal.querySelector('[data-rank-predictor-score]');
    if (scoreEl) scoreEl.textContent = prediction.yourScore + '%';
    
    // Update confidence/message
    const messageEl = modal.querySelector('[data-rank-predictor-improvement]');
    if (messageEl) {
      messageEl.textContent = prediction.message;
      messageEl.style.color = prediction.isInsufficientData ? '#ffa726' : '#66bb6a';
      messageEl.style.fontSize = '12px';
      messageEl.style.fontWeight = '500';
    }
    
    // Add additional stats display
    const statsInfo = modal.querySelector('[data-rank-predictor-stats-info]');
    if (statsInfo) {
      statsInfo.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px; font-size: 12px;">
          <div style="padding: 8px; background: rgba(99,102,241,0.1); border-radius: 6px; text-align: center;">
            <div style="color: rgba(255,255,255,0.6); font-size: 11px;">Tests Completed</div>
            <div style="color: #6366f1; font-weight: 700; font-size: 16px;">${prediction.totalTests}</div>
          </div>
          <div style="padding: 8px; background: rgba(102,187,106,0.1); border-radius: 6px; text-align: center;">
            <div style="color: rgba(255,255,255,0.6); font-size: 11px;">Attempt Rate</div>
            <div style="color: #66bb6a; font-weight: 700; font-size: 16px;">${prediction.attemptRate}%</div>
          </div>
          <div style="padding: 8px; background: rgba(255,193,7,0.1); border-radius: 6px; text-align: center;">
            <div style="color: rgba(255,255,255,0.6); font-size: 11px;">Confidence</div>
            <div style="color: #ffc107; font-weight: 700; font-size: 16px;">${prediction.confidence}%</div>
          </div>
          <div style="padding: 8px; background: rgba(244,67,54,0.1); border-radius: 6px; text-align: center;">
            <div style="color: rgba(255,255,255,0.6); font-size: 11px;">Avg Questions</div>
            <div style="color: #f44336; font-weight: 700; font-size: 16px;">${prediction.questionsAttempted}/10</div>
          </div>
        </div>
      `;
    }
    ...
    console.error('[Rank Predictor Update] Error:', e);
  }
}
...
function hideAllScreensForMockTest() {
  try {
    // Only hide specific visible content areas, NOT the main app container
    ...
    // Hide any visible modal-like elements (except mock test modals)
    const modals = document.querySelectorAll('[class*="modal"]:not(#mockTestModal):not(#mockTestFullscreenModal):not(#mockTestResultsModal)');
    modals.forEach((modal, idx) => {
      const id = modal.id || `temp-modal-${idx}`;
      if (!originalDisplayStates[id]) {
        originalDisplayStates[id] = {
    ...
    console.log('[MockTest] Screens hidden successfully');
  } catch(e) {
    console.error('[MockTest] Error hiding screens:', e);
  }
}

function showAllScreensAfterMockTest() {
  try {
    // Restore all previously hidden elements
    Object.keys(originalDisplayStates).forEach(id => {
      const el = document.getElementById(id);
      if (el && originalDisplayStates[id]) {
        el.style.display = originalDisplayStates[id].display || '';
        el.style.visibility = originalDisplayStates[id].visibility || '';
        el.style.pointerEvents = originalDisplayStates[id].pointerEvents || '';
      }
    });
    
    // Clear the stored states
    Object.keys(originalDisplayStates).forEach(key => {
      delete originalDisplayStates[key];
    });
    ...
    const mockModal = document.getElementById('mockTestFullscreenModal');
    const resultsModal = document.getElementById('mockTestResultsModal');
    if (mockModal) mockModal.style.display = 'none';
    if (resultsModal) resultsModal.style.display = 'none';
    
    console.log('[MockTest] All screens restored successfully');
  } catch(e) {
    console.error('[MockTest] Error showing screens:', e);
  }
}
...
function openAddonModal(type) {
  ...
  modal.innerHTML = `
    <div style="background:linear-gradient(135deg,#0f0c1f,#1a1435);border:1px solid rgba(108,99,255,0.35);border-radius:20px;padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 0 60px rgba(108,99,255,0.15);">
      ...
      <div style="font-size:13px;color:var(--text-secondary);margin-bottom:18px;">${isVision ? 'Advanced AI analysis of images, PDFs & handwritten notes' : 'Deepest AI reasoning with multi-step explanations'}</div>
      <ul style="text-align:left;font-size:12px;color:var(--text-secondary);list-style:none;padding:0;margin:0 0 20px 0;display:flex;flex-direction:column;gap:6px;">
        ${isVision
      ...
      <div style="font-size:28px;font-weight:800;color:#6C63FF;margin-bottom:4px;">₹${ADDON_PRICE} <span style="font-size:13px;font-weight:400;color:var(--text-secondary);">one-time</span></div>
      <div style="font-size:11px;color:var(--text-secondary);margin-bottom:20px;">Lifetime access · No subscription</div>
      ...
      <button onclick="document.getElementById('addonModal').remove()" style="width:100%;padding:10px;background:transparent;color:var(--text-secondary);border:1px solid rgba(108,99,255,0.2);border-radius:10px;font-size:13px;cursor:pointer;">
        Maybe Later
      </button>
      <div style="margin-top:14px;font-size:11px;color:var(--text-muted);">🔒 Secured by Cashfree Payments</div>
    </div>
  `;
  document.body.appendChild(modal);
...
function openV4ProModal() {
  ...
  modal.innerHTML = `
    <div style="background:linear-gradient(135deg,#0a0520,#150a30,#0a0520);border:1px solid rgba(255,107,157,0.4);border-radius:24px;padding:30px 24px;max-width:380px;width:100%;text-align:center;box-shadow:0 0 80px rgba(255,107,157,0.15),0 0 40px rgba(108,99,255,0.1);">
      ...
      <div style="font-size:13px;color:var(--text-secondary);margin-bottom:20px;line-height:1.6;">The most powerful DeepSeek model — 1M context window, best-in-class reasoning for tough SSC/Board questions</div>
      ...
      <div style="background:rgba(108,99,255,0.08);border:1px solid rgba(108,99,255,0.2);border-radius:12px;padding:12px;margin-bottom:18px;">
        <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">Why ₹149/month?</div>
        <div style="font-size:12px;color:var(--text-secondary);line-height:1.5;">V4 Pro costs 12× more than V4 Flash on the API. This plan covers those costs so you get flagship AI at the lowest sustainable price.</div>
      </div>
...
window.openAiDisclaimerModal = openAiDisclaimerModal;
window.closeAiDisclaimerModal = closeAiDisclaimerModal;
...
function getCoins() {
  ...
  // ✅ FIX: Priority order — Firestore is synced to localStorage by loadCoinsFromFirebase()
  // 1. Battle Arena per-uid (already synced from Firestore via loadCoinsFromFirebase)
  try {
...
function setCoins(n) {
  ...
  // ✅ FIX #2: Also sync to Firestore so it persists across devices
  if (uid && uid !== 'guest' && window._firebaseDb && window._firebaseFns) {
    try {
      const { doc, setDoc } = window._firebaseFns;
      setDoc(doc(window._firebaseDb, 'userCoins', uid), {
        coins: amount,
        lastUpdated: Date.now()
      }, { merge: true }).catch(() => {
        // Silently fail — coins saved to localStorage at minimum
      });
    } catch(e) {
      // Silently fail
    }
...
function initApp() {
  ...
  // Hide motion animation when user starts typing
  dom.messageInput.addEventListener('input', function() {
    if (this.value.trim().length > 0) {
      const motionWrapper = document.getElementById('ai-motion-wrapper');
      if (motionWrapper && motionWrapper.style.display !== 'none') {
        motionWrapper.style.transition = 'opacity 0.3s ease-out';
        motionWrapper.style.opacity = '0';
        setTimeout(() => {
          motionWrapper.style.display = 'none';
          motionWrapper.style.visibility = 'hidden';
  ...
  // Show/hide premium upgrade screen based on user status
  updatePremiumUpgradeScreen();
  ...
  // ── FEATURE MODAL BACKDROP CLICK HANDLERS ─────────────────
  // Allow closing modals by clicking on the backdrop (outside the content)
  const featureModals = ['mockTestModal', 'rankPredictorModal', 'analyticsModal'];
  featureModals.forEach(modalId => {
    ...
    onAuthStateChanged(window._firebaseAuth, async (fbUser) => {
      ...
      if (fbUser) {
        ...
        if (!state.user) {
          ...
          loadUserState(uid);
          // ✅ FIX #1 & #2: Load Premium & Coins from Firebase on login
          await loadPremiumStatusFromFirebase(uid);
          await loadCoinsFromFirebase(uid);
          // ✅ Load XP from Firebase on login
...
const TOTAL_COMPETITORS = 850000; // 8.5 lakhs

window.openRankPredictorModal = function() {
  try {
    // Save the previous state before opening rank predictor
    if (typeof ModalStateManager !== 'undefined') {
      ModalStateManager.savePreviousState();
      ModalStateManager.currentModal = 'rankPredictor';
    }
    
    const modal = document.getElementById('rankPredictorModal');
    if (!modal) {
      console.error('Rank Predictor modal element not found');
      return;
    }
  ...
  let mockTestData = [];
  try {
    const historyKey = `sscai_u:${userID}:mock_history`;
    const stored = localStorage.getItem(historyKey);
    mockTestData = stored ? JSON.parse(stored) : [];
  } catch(e) {}
  
  // Get XP data
  const xpData = MockTestModule.getUserXP(userID);
  
  // Calculate statistics
  const rankEl = document.querySelector('[data-rank-predictor-rank]');
  const percentileEl = document.querySelector('[data-rank-predictor-percentile]');
  const scoreEl = document.querySelector('[data-rank-predictor-score]');
  const cutoffEl = document.querySelector('[data-rank-predictor-cutoff]');
  const improvementEl = document.querySelector('[data-rank-predictor-improvement]');
  const statsInfoEl = document.querySelector('[data-rank-predictor-stats-info]');
  
  if (mockTestData.length < 3) {
    rankEl.textContent = '---';
    percentileEl.textContent = '---%';
    scoreEl.textContent = '---%';
    cutoffEl.textContent = '---';
    improvementEl.textContent = 'Take more tests for accurate prediction';
    statsInfoEl.innerHTML = '<div style="padding: 12px; text-align: center; color: rgba(255,255,255,0.5); font-size: 13px;">Complete at least 3 mock tests to get accurate rank prediction</div>';
  } else {
    // Calculate average accuracy
    const accuracies = mockTestData.map(t => t.accuracy || 0);
    const avgAccuracy = Math.round(accuracies.reduce((a, b) => a + b, 0) / mockTestData.length);
    
    // Calculate percentile
    const percentile = Math.min(99, Math.max(1, Math.round(50 + (avgAccuracy - 45) * 1.5)));
    
    // Calculate rank
    const rank = Math.round(TOTAL_COMPETITORS * (100 - percentile) / 100);
    
    // Calculate improvement
    const firstThreeAvg = accuracies.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
    const lastThreeAvg = accuracies.slice(-3).reduce((a, b) => a + b, 0) / 3;
    const improvement = Math.round(lastThreeAvg - firstThreeAvg);
    
    // Calculate cutoff
    const cutoffScore = Math.max(20, Math.round(avgAccuracy * 0.6));
    
    // Get category-wise stats
    const categoryPerformance = {};
    mockTestData.forEach(test => {
      const category = test.examType || 'General';
      if (!categoryPerformance[category]) categoryPerformance[category] = [];
      categoryPerformance[category].push(test.accuracy || 0);
    });
    ...
    Object.entries(categoryPerformance).forEach(([category, scores]) => {
      ...
      if (avg < 50) weakAreas.push({ category, score: avg });
      if (avg >= 80) strongAreas.push({ category, score: avg });
    });
    ...
    // Update UI
    rankEl.textContent = rank.toLocaleString('en-IN');
    percentileEl.textContent = percentile + '%';
    scoreEl.textContent = avgAccuracy + '%';
    cutoffEl.textContent = cutoffScore + '%';
    
    if (improvement > 0) {
      improvementEl.textContent = `📈 +${improvement}% improvement`;
      improvementEl.style.color = '#4caf50';
    } else if (improvement < 0) {
      improvementEl.textContent = `📉 ${improvement}% decline`;
      improvementEl.style.color = '#ff6b6b';
    } else {
      improvementEl.textContent = 'Stable performance';
    }
    
    let statsHTML = '<div style="padding: 12px; font-size: 12px; color: rgba(255,255,255,0.7);">';
    statsHTML += `<div style="margin-bottom: 8px;">📊 Tests Analyzed: ${mockTestData.length}</div>`;
    statsHTML += `<div style="margin-bottom: 8px;">⭐ Best Score: ${Math.max(...accuracies)}%</div>`;
    statsHTML += `<div style="margin-bottom: 8px;">🎖️ Level: ${xpData?.level || 1}</div>`;
    statsHTML += `<div>🏆 XP Points: ${xpData?.total || 0}</div>`;
    statsHTML += '</div>';
    
    if (strongAreas.length > 0) {
      statsHTML += '<div style="padding: 12px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 12px;">';
      statsHTML += '<div style="color: #4caf50; font-weight: 600; margin-bottom: 6px;">✓ Strong Areas:</div>';
      strongAreas.forEach(area => {
        statsHTML += `<div style="margin: 4px 0; color: rgba(255,255,255,0.7);">${area.category}: ${area.score}%</div>`;
      });
      statsHTML += '</div>';
    }
    ...
    if (weakAreas.length > 0) {
      statsHTML += '<div style="padding: 12px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 12px;">';
      statsHTML += '<div style="color: #ff6b6b; font-weight: 600; margin-bottom: 6px;">⚠️ Areas to Improve:</div>';
    ...
    // Ensure modal is properly displayed with both class and inline styles
    modal.classList.add('active');
    modal.style.display = 'flex !important';
    modal.style.visibility = 'visible';
    modal.style.opacity = '1';
    modal.style.pointerEvents = 'auto';
    modal.style.zIndex = '99992';
  } catch (e) {
    console.error('[RankPredictorModal] Error:', e);
    alert('Error opening Rank Predictor. Please try again.');
  }
};
...
window.openAnalyticsModal = function() {
  ...
  const testsEl = document.querySelector('[data-analytics-tests]');
  const accuracyEl = document.querySelector('[data-analytics-accuracy]');
  const xpEl = document.querySelector('[data-analytics-xp]');
  const improvementEl = document.querySelector('[data-analytics-improvement]');
  const categoryEl = document.querySelector('[data-category-breakdown]');
  
  if (mockTestData.length === 0) {
    testsEl.textContent = '0';
    accuracyEl.textContent = '0%';
    xpEl.textContent = xpData?.total || '0';
    improvementEl.textContent = '0%';
    categoryEl.innerHTML = '<div style="padding: 12px; text-align: center; color: rgba(255,255,255,0.5);">Take mock tests to see analytics</div>';
  } else {
    const accuracies = mockTestData.map(t => t.accuracy || 0);
    const avgAccuracy = Math.round(accuracies.reduce((a, b) => a + b, 0) / mockTestData.length);
    
    testsEl.textContent = mockTestData.length;
    ...
    Object.entries(categoryPerformance).forEach(([category, scores]) => {
      const avg = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
      const width = avg;
      let color = avg >= 80 ? '#4caf50' : avg >= 60 ? '#ffa726' : '#ff6b6b';
      
      categoryHTML += `
        <div style="margin-bottom: 12px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
            <span style="font-size: 12px; color: rgba(255,255,255,0.8);">${category}</span>
            <span style="font-size: 12px; font-weight: 600; color: ${color};">${avg}%</span>
          </div>
  ...
  modal.style.visibility = 'visible';
    ...
    console.error('[AnalyticsModal] Error:', e);
    alert('Error opening Analytics. Please try again.');
  }
};
window.closeRankPredictorModal = function() {
  try {
    ...
    console.error('[RankPredictorModal] Close error:', e);
  }
};
...
window.closeAnalyticsModal = function() {
  try {
    ...
    console.error('[AnalyticsModal] Close error:', e);
  }
};


console.info('[Analytics] Rank Predictor & Analytics enabled');

// ═══════════════════════════════════════════════════════════════════════════════
// ✅ FIX #1 & #2: Sync Premium Status & Coins when App Resumes
// ═══════════════════════════════════════════════════════════════════════════════
// Listen for visibility change (when user switches back to the app)
document.addEventListener('visibilitychange', async () => {
  if (!document.hidden && state.user) {
    // App came to foreground - reload premium and coins from Firestore
    const uid = state.user.uid;
    await loadPremiumStatusFromFirebase(uid);
    await loadCoinsFromFirebase(uid);
  }
...
// Also sync on focus event for broader device support
